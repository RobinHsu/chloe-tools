...
gulp.task(
  ...
  gulp.series(done => {
    ...
    const args = [tslintBin, '-c', tslintConfig, 'src/**/*.tsx'];
...
gulp.task(
  ...
  gulp.series(done => {
    const tslintBin = require.resolve('tslint/bin/tslint');
    const tslintConfig = path.join(__dirname, './tslint.json');
    const args = [tslintBin, '-c', tslintConfig, 'src/**/*.tsx', '--fix'];
    runCmd('node', args, done);
  })
);
  ...
  let stream = js.pipe(babel(babelConfig)).pipe(
    through2.obj(function z(file, encoding, next) {
      ...
      if (file.path.match(/(\/|\\)style(\/|\\)index\.js/)
        || file.path.match(/(\/|\\)style(\/|\\)themes(\/|\\)[a-z]*\/index\.js$/)) {
...
function compile(modules) {
    ...
    .pipe(
      through2.obj(function(file, encoding, next) {
        ...
        if (
          ...
          || file.path.match(/(\/|\\)style(\/|\\)themes(\/|\\)[a-z]*(\/|\\)index\.scss$/)
        ) {
  ...
  const fonts = gulp.src(['src/**/fonts/iconfont.*'], { base: 'src' })
    .pipe(gulp.dest(modules === false ? esDir : libDir));
  ...
  return merge2([scss, fonts, tsFilesStream, tsd, assets]);
}

function publish(tagString, done) {
  let args = ['publish', '--with-chloe-tools', '--access=public'];
  if (tagString) {
  ...
  gulp.series(done => {
    ...
    if (npmArgs) {
      for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
        if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-chloe-tools') < 0) {
          reportError();
          done(1);
          return;
        }
